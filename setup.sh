...
install_pip_package() {
    PACKAGE=$1
    log_info "Verificando e instalando $PACKAGE (pip)..."
    ...
    if [ $? -ne 0 ]; then
        ...
        log_info "$PACKAGE já está instalado via pip."
    fi
}
...
log_info "Iniciando instalação de dependências Python..."
...
log_info "Para executar o PCAPCracker Pro, use: python3 $PCAP_CRACKER_PRO_PY"
